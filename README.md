...
Функция `transaction_amount(transaction)` в модуле `external_api.py` 
служит для вывода суммы транзакции в рублях. Если операция проходила 
...
Реализованы функции для работы с описанием транзакций через регулярные выражения и библиотеку `re`
Переписан модуль `main.py`, в котором появилось взаимодействие с пользователем через функцию `main()`
...
```
## Создание лог-файлов
С 16.02.25 в программе при помощи декоратора ` log(filename)`
реализована запись данных о ходе выполнения функций:
- время начала и конца исполнения
- имя функции
- результаты, полученные в ходе операций
- возникшие в ходе исполнения ошибки
...
Тесты находятся в папке `tests/`, а результаты покрытия тестами кода -- в `/htmlcov/index.html`
